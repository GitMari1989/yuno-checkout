...
const seamlessExternalButtonsPage = path.join(
  __dirname,
  "vanilla/pages/checkout-seamless-external-buttons.html"
)
...
app.get("/sdk-web/healthy", (_req, res) => res.sendStatus(200))

app.get("/", (_req, res) => res.sendFile(indexPage))
app.get("/checkout", (_req, res) => res.sendFile(checkoutPage))
app.get("/checkout/lite", (_req, res) => res.sendFile(checkoutLitePage))
app.get("/checkout/seamless", (_req, res) => res.sendFile(seamlessCheckoutPage))
app.get("/checkout/seamless/lite", (_req, res) => res.sendFile(seamlessCheckoutLitePage))
app.get("/checkout/seamless/external-buttons", (_req, res) => res.sendFile(seamlessExternalButtonsPage))
app.get("/checkout/secure-fields", (_req, res) => res.sendFile(checkoutSecureFieldsPage))
app.get("/status", (_req, res) => res.sendFile(statusPage))
app.get("/status-lite", (_req, res) => res.sendFile(statusLitePage))
app.get("/enrollment-lite", (_req, res) => res.sendFile(enrollmentLitePage))
app.get("/full-features", (_req, res) => res.sendFile(fullFeatures))
app.get("/checkout/payment-methods-unfolded", (_req, res) => res.sendFile(paymentMethodsUnfolded))

app.get("/public-api-key", (_req, res) => res.json({ publicApiKey: PUBLIC_API_KEY }))

/**
 * Payment methods for checkout session
 */
app.get("/payment-methods/:checkoutSession", async (req, res) => {
  try {
    ...
    return res.status(err?.status || 500).json({ error: String(err?.message || err), data: err?.data || null })
  }
})

/**
 * ✅ NEW: get payment by id (so frontend can poll and close UI safely)
 */
app.get("/payments/:paymentId", async (req, res) => {
  try {
    ...
    const url = `${API_URL}/v1/payments/${paymentId}`
    ...
    console.log("[ERR] GET /payments/:paymentId:", err?.message, err?.data || "")
    return res.status(err?.status || 500).json({ error: String(err?.message || err), data: err?.data || null })
  }
})
